...
_TOP_HELP = """CrowdStrike-MDRユーザの追加作業をおこないます
"""
...
def _copy_all_config_to_backup(config):
	...
	shutil.copyfile(config["splunk_devices"],
		_BACKUP_DIR+"/cs_devices.csv")
	shutil.copyfile(config["input_conf"],
		_BACKUP_DIR+"/inputs.conf")
	shutil.copyfile(config["app_creds_share"],
		_BACKUP_DIR+"/oauth.json")
	shutil.copyfile(config["app_creds_share2"],
		_BACKUP_DIR+"/threat_graph.json")
	shutil.copytree(config["app_creds_eachdir"],
		_BACKUP_DIR+"/credentials")

def _copy_all_config(work_type, config):
	work_conf_dir = "{}/{}/config".format(_WORK_DIR, work_type)
	shutil.copyfile(config["splunk_devices"],
		work_conf_dir+"/cs_devices.csv")
	shutil.copyfile(config["input_conf"],
		work_conf_dir+"/inputs.conf")
	shutil.copyfile(config["app_creds_share"],
		work_conf_dir+"/oauth.json")
	shutil.copyfile(config["app_creds_share2"],
		work_conf_dir+"/threat_graph.json")
	shutil.copytree(config["app_creds_eachdir"],
...
def _parse_api_conf_current(config, api_type="oauth"):
	results = {}
	with open(config["app_creds_share"]) as f:
		creds = json.load(f)
	for customer,each in creds.items():
		each["file_name"] = "{}.json".format(api_type)
		each["is_share"]  = True
		results[customer]=each
	for each in glob.glob(config["app_creds_eachdir"]+"/*/{}.json".format(api_type)):
		file_name = re.search("(?i)\w+/\w+\.json", each).group()
		customer = file_name.split("/")[0]
		with open(each) as f:
			each = json.load(f)
...
def _parse_api_conf(work_dir, api_type="oauth"):
	results = {}
	with open("{}/config/{}.json".format(work_dir, api_type)) as f:
		creds = json.load(f)
	for customer,each in creds.items():
		each["file_name"] = "share_{}".format(api_type)
		each["is_share"]  = True
		results[customer]=each
	for each in glob.glob("{}/config/credentials/*/{}.json".format(work_dir, api_type)):
		file_name = re.search("(?i)\w+/\w+\.json", each).group()
		customer = file_name.split("/")[0]
		with open(each) as f:
			each = json.load(f)
...
def _get_index(work_dir, customer_name):
	index_fname = work_dir+"/indexinfo.json"
	if os.path.exists(index_fname):
		with open(index_fname) as f:
			index_info = json.load(f)
		res = yes_no_input("インデックス情報は以下であってますか？\n{}".format(index_info["index"]))
		if res:
			return index_info["index"]

	inputs = _parse_input_conf(work_dir+"/config/inputs.conf")
	for key in inputs.keys():
		each = inputs[key]
		eachkeys = each.keys()
		if not "index" in eachkeys:
			continue
		res = yes_no_input(
			"インデックス情報は以下であってますか？\nindex:{}\naccount:{}".format(
				each["index"], each["account"]))
		if res:
			with open(index_fname, "w") as wf:
				json.dump({"index" : each["index"]}, wf, indent=4)
			return each["index"]
	logger.error("削除対象のインデックスが選択されませんでした。")
	exit(0)
...
def _diff_csapi(api_type, work_dir, config):
	before = _parse_api_conf(work_dir, api_type)
	after  = _parse_api_conf_current(config)
	add_list = []
	del_list = []
	for each in before.keys():
		if not each in after:
			info = before[each]
			del_list.append( { "customer_name" : info["customer_name"] } )
	for each in after.keys():
		if not each in before:
			info = after[each]
			add_list.append( { "customer_name" : info["customer_name"] } )
	return { "add" : add_list, "del" : del_list }

def _check_api_add(api_type, customer_name, index, config, work_dir):
	result = _diff_csapi(api_type, work_dir, config)
	print json.dumps(result, indent=4)
	if len(result["del"])>0:
...
def _check_cbapi_delete(api_type, customer_name, index, config, work_dir):
	result = _diff_csapi(api_type, work_dir, config)
	print json.dumps(result, indent=4)
	if len(result["add"])>0:
...
def _check_customer_add(customer_name, index, config, work_dir):
	...
	before = _parse_customer_csv("{}/config/cs_devices.csv".format(work_dir))
...
def _check_customer_delete(customer_name, config, work_dir):
	after  = _parse_customer_csv(config["splunk_devices"])
	before = _parse_customer_csv("{}/config/cs_devices.csv".format(work_dir))
	result = _diff_devices_conf(before, after)
	if len(result["add"])>0:
...
def _get_api_conf(customer, index, work_dir):
	conf_oauth        = _parse_api_conf(work_dir, "oauth")
	conf_threat_graph = _parse_api_conf(work_dir, "threat_graph")
	return {"oauth"        : conf_oauth.get(customer),
			"threat_graph" : conf_threat_graph.get(customer) }
...
def del_user(args, config):
	...
	# 入力設定の削除
	username_of_index = index.split("_")[1]
	res = raw_input(procedure.delete_input.format(hostname, index, username_of_index, target["customer"]))
	res_input_delete = _check_input_delete(index, config, work_dir)
	_write_current("1.delete_input", args.type)

	# cs_devices.csvから削除
	res = raw_input(procedure.delete_customer_csv.format(target["customer"]))
	res_customer_delete = _check_customer_delete(target["customer"], config, work_dir)
	_write_current("2.delete_customer", args.type)

	# Splunk内に保管しているCBのAPI設定を削除
	api_conf =  _get_api_conf(target["customer"], index, work_dir)
	if api_conf["oauth"] is None or api_conf["threat_graph"] is None:
		logger.error("対象ユーザの設定がすでに存在しません。")
		res_customer_delete = "削除対象のユーザなし"
	else:
		_each_api_conf = api_conf["oauth"]
		if _each_api_conf["is_share"]:
			res = raw_input(procedure.delete_csapi_oauth_conf_by_share.format(target["customer"], index))
		else:
			res = raw_input(procedure.delete_api_each_file.format(_each_api_conf["file_name"], target["customer"], index))
		_each_api_conf = api_conf["threat_graph"]
		if _each_api_conf["is_share"]:
			res = raw_input(procedure.delete_csapi_threat_graph_conf_by_share.format(target["customer"], index))
		else:
			res = raw_input(procedure.delete_api_each_file.format(_each_api_conf["file_name"], target["customer"], index))
		res_cbapi_delete = _check_cbapi_delete("oauth", target["customer"], index, config, work_dir)
...
def add_user(args, config): #TODO
	...
	# cs_devices.csvへの追加
	oauth_id   = raw_input("OAuthのclient_idを入力して下さい:")
	oauth_pass = raw_input("OAuthのsecretを入力して下さい:")
	tg_id      = raw_input("ThreatGraphのusernameを入力して下さい:")
	tg_pass    = raw_input("ThreatGraphのpasswordを入力して下さい:")
	cid        = raw_input("CSのCIDを入力して下さい(SensorDownload画面で確認):")
	edr_host   = raw_input("CrowdStrikeログオン時のホスト名を入力して下さい(例:falcon.crowdstrike.com):")
	res = raw_input(procedure.add_customer.format(target["customer"], appid, cid, target["sender"], edr_host))
	res_customer_add = _check_customer_add(target["customer"], index, config, work_dir)
	_write_current("2.add_customer", args.type)

	# CSのAPI設定を追加
	procedure_for_csapi(edr_host, index, target["customer"], oauth_id, oauth_pass, tg_id, tg_pass)
	res_api_add = _check_api_add("oauth", target["customer"], index, config, work_dir)
	_write_current("3.add_api", args.type)

	# 入力設定を追加
	stream_user = raw_input("StreamingAPIのusernameを入力して下さい:")
	stream_pass = raw_input("StreamingAPIのsecretを入力して下さい:")
	res = raw_input(procedure.add_input.format(
		hostname, edr_host, appid, index, target["customer"],
		stream_user, stream_pass))
...
def procedure_for_csapi(edr_host, index, customer, oauth_id, oauth_pass, tg_id, tg_pass):
	# CSのAPI設定を追加
	if edr_host == "falcon.crowdstrike.com":
		res = raw_input(procedure.add_csapi.format(
			customer, oauth_id, oauth_pass, tg_id, tg_pass))
	else:
		res = raw_input(procedure.add_csapi2.format(
			customer, oauth_id, oauth_pass, tg_id, tg_pass))
