...
diff work/"$work"/config/cb_devices.csv backup/cb_devices.csv >> backup/lastcheck.txt

echo "***inputs.confの差分チェック結果***" >> backup/lastcheck.txt
diff work/"$work"/config/inputs.conf backup/inputs.conf >> backup/lastcheck.txt

echo "***cbdefense.json(共有api設定)の差分チェック結果***" >> backup/lastcheck.txt
diff work/"$work"/config/cbdefense.json backup/cbdefense.json >> backup/lastcheck.txt

echo "***CBAPI(credentials)(個別api設定)の差分チェック結果***" >> backup/lastcheck.txt
