...
class GraphBuilder(object):
	...
	def _table_by_incident(self):
		_INCIDENT_CSV_ = CURR_DIR+"/srcfiles"
		def to_incident_alert_table():
			results = {}
	...
	def _table_by_malwaretype(self):
		...
		def get_malware_list(rawdata):
			result = []
			if rawdata["alert_detail"].get("malware_detail"):
				for each in rawdata["alert_detail"]["malware_detail"]:
					malname = each["name"]
					if isinstance(malname, basestring):
						malname = malname
					else:
						malname = "その他"
					result.append(malname)
			return result
		...
		def host_counts_by_malwaretype(alerts):
			...
			for alert in alerts:
				...
				if rawdata:
					malwares = get_malware_list(rawdata)
				else:
					continue
				for mal_type in malwares:
					severity = self._convert_severity(alert["severity"])
					logger.debug( "{} -> {}:{}".format(
