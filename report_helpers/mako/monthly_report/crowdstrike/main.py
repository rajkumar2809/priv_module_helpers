...
_ORIG_SEV_FILE_DIR  = DATA_DIR+"/crowdstrike_origin"
...
def _split_by_customers(alerts, stats6month, statsByOrigSev, incidents, customers, target):
	result = {}
	...
	for customer in result.keys():
		if customer in statsByOrigSev:
			result[customer]["statsByOrigSev"] = statsByOrigSev[customer]
		else:
			result[customer]["statsByOrigSev"] = {}
	for customer in result.keys():
...
def parse_orig_severity_csv():
	result = {}
	for each in glob.glob(_ORIG_SEV_FILE_DIR+"/*.csv.gz"):
		eachstats = file_util.parse_csv_gzip(each)
		for eachrow in eachstats:
			customer = eachrow["customer_name"]
			severity = eachrow["severity"].lower()
			alertnum = eachrow["count"]
			if(customer in result):
				res = result[customer]
				if severity in res:
					res[severity] += int(alertnum)
				else:
					res[severity] = int(alertnum)
			else:
				result[customer] = {}
				result[customer][severity] = int(alertnum)
	if len(result) is 0:
		print("!!! You must put Crowdstrike Original Severity Stats")
		exit(0)
	return result
...
def main():
	...
	statsByOrigSev = parse_orig_severity_csv()
	incidents = parse_incident_csv()
	alerts_by_customer = _split_by_customers(
			alerts, stats6month, statsByOrigSev, incidents,
