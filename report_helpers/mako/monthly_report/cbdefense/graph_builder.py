...
import os, sys
import json, copy, base64, re, copy, glob
...
class GraphBuilder(object):
	...
	@classmethod
	def init_module(cls, report_dir, data_dir, month_diff, excludes):
		cls.REPORT_DIR = report_dir
		cls.DATA_DIR = data_dir
		cls._TARGET_MONTH = month_diff
		cls._EXCLUDE_DATE = excludes
		malware_dir = cls.DATA_DIR+"/malwares"
		malware_list_file = malware_dir+"/malwares.csv"
		malwares = {}
		if os.path.exists(malware_list_file):
			tmp = file_util.parse_csv(malware_list_file)
			for each in tmp:
				malwares[each["malhash"]]=each["malname"]
		for each in glob.glob(malware_dir+"/appends/*.csv"):
			tmp = file_util.parse_csv(each)
			for each in tmp:
				try:
					if each.get("malname"):
						malname = each["malname"].strip("/")
						if malname and len(malname) is not 0:
							malwares[each["malhash"]] = malname
				except Exception as e:
					logger.warning("error happened at malwares 1 line:{}".format(str(each)))
					logger.exception(e)
		for each in glob.glob(malware_dir+"/appends/*.csv.gz"):
			tmp = file_util.parse_csv_gzip(each)
			for each in tmp:
				try:
					if each.get("malname"):
						malname = each["malname"].strip("/")
					...
					logger.warning("error happened at malwares 1 line:{}".format(str(each)))
					logger.exception(e)
		hdrs = ["malhash", "malname"]
		rawmallist = [ [k, v] for k,v in malwares.items() ]
		file_util.write_csv(malware_list_file, rawmallist, hdrs)
		cls.malware_names = malwares
	...
	def _table_by_ostype(self):
		...
		def severity_counts_by_ostype(alerts):
			...
			for alert in alerts:
				_os = alert["os"].lower()
				os_type = "unknown" if _os in ( None , "null" , "" , "-") else _os.replace("null" , "unknown")
				if os_type and len(os_type)>0: #TODO for blank OS fields
					severity = self._convert_severity(alert["severity"])
					logger.debug( "{} -> {}:{}".format(
	...
	def _table_by_malwaretype(self):
		...
		def is_malware_alert(alert):
			if alert["severity"] == "-":
				return False
			elif alert["alert_type"].lower() in ("malware", "pup"):
				return True
			if   re.search(pt, alert["process_rep"]):
				return True
			elif re.search(pt, alert["target_rep"]):
				return True
			else:
				return False
		...
		def host_counts_by_malwaretype(alerts):
			results = {}
			for alert in alerts:
				mal_type = alert["malware_type"]
				if mal_type == "その他":
					rawdata = alert.get("rawdata")
					if rawdata and rawdata["alert_summary"]["malware_info"]:
						for each in rawdata["alert_summary"]["malware_info"]:
							eachhash = each["sha256Hash"]
							if isinstance(eachhash, basestring) and re.search(r"[0-9|a-z|A-Z]{64}", eachhash):
								if eachhash in self.malware_names:
									malname = self.malware_names[eachhash]
									if mal_type == "その他":
										mal_type = malname
									elif not malname in mal_type:
										mal_type = "{}\n{}".format(mal_type, malname)
					if mal_type == "その他":
						pt = r"(?i)_?(malware|pup)$"
						if (re.search(pt, alert["process_rep"]) and
							not alert["ps"] == "null"):
							mal_type = alert["ps"]
						elif (re.search(pt, alert["target_rep"]) and
							not alert["target"] == "null"):
							mal_type = alert["target"]
						elif not self.language == "japanese":
							mal_type = "etc.."
				else:
					mal_type = mal_type.strip("/")
				pt = r"(?i)^the\s+file\s+\"(.*)\"\s+(attempted\s+to\s*)?run.*\son\s*(macos|windows|linux)"
				if re.search(pt, mal_type):
					mal_type = re.search(pt, mal_type).groups()[0]
				severity = self._convert_severity(alert["severity"])
				logger.debug( "{} -> {}:{}".format(
		...
		logger.debug("make table by malware type.")
		#alerts = [ each for each in self.alerts if is_malware_alert(each) ]
