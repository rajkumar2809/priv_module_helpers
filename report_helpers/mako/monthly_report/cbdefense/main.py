...
TOP_HELP ='''
make monthly report for cbdefense.
...
def _init_environment(month_diff=-1, excludes=None):
	...
	#_graph.set_dir( REPORT_DIR, DATA_DIR)
	#_graph.set_excludes(excludes)
	#_graph.set_month(int(month_diff))
	_graph.init_module( REPORT_DIR, DATA_DIR, int(month_diff), excludes )
...
def _split_by_customers(alerts, stats6month, incidents, customers, target):
	...
	for customer in result.keys():
		keys = []
		for eachStats6month in stats6month:
			keys = [ each for each in eachStats6month if each.endswith(":"+customer) ]
...
def parse_alert_csv():
	...
	for each in glob.glob(_MONTHLY_FILE_DIR+"/*.csv.gz"):
		...
		for alert in eachalerts:
			...
			try:
				alert["rawdata"] = json.loads(alert["_raw"])
			except Exception as e:
				logger.error("json parse error at {}".format(key))
			alerts[key] = alert
...
def main():
	...
	alerts_by_customer = _split_by_customers(
			alerts, stats6month, incidents, config["customers"], target_customer)
