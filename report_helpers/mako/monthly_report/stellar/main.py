...
CSV_TOP_BY_APP_CHART = "top_by_appname.csv"
CSV_TOP_BY_SRCIP_CHART = "top_by_srcip.csv"
CSV_TOP_BY_DSTIP_CHART = "top_by_dstip.csv"
CSV_TOP_BY_DSTIP_GEO_CHART = "top_by_dst_geo.csv"
...
TOP_HELP ='''
make monthly report for stellar.
...
def _set_argement():
	parser.add_argument('src',
		...
		help="search target splunk, default is file.")
	...
	parser.add_argument('--exclude_date',
		default=[],
		help='exclude date for monthly report.') 
	parser.add_argument('--pdf',
		nargs="?", const=True, 
		help='convert to pdf file. default is true.') 
...
def _split_by_customers(alerts, stats6month, customers, target):
	result = {}
	...
	for each in alerts:
		...
		if each["risklevel"] == "middle":
			each["risklevel"] = "medium"
			if "severity" in each:
				each["aella_severity"] = "severity"
			each["severity"] = each["risklevel"]
			each["date"] = each["detect_time"].split(" ")[0]
...
def write_all_table2csv(customer_name, tableinfo):
	...
	each = tableinfo["monthlychart_table"]
	if each:
		file_name = csv_dir+"/"+CSV_MONTHLY_CHART
		_write_csv(file_name, each["header"], each["fields"])

	each = tableinfo["topnum_by_app_table"]
	if each:
		file_name = csv_dir+"/"+CSV_TOP_BY_APP_CHART
		_write_csv(file_name, each["header"], each["fields"])

	each = tableinfo["topnum_by_srcip_table"]
	if each:
		file_name = csv_dir+"/"+CSV_TOP_BY_SRCIP_CHART
		_write_csv(file_name, each["header"], each["fields"])

	each = tableinfo["topnum_by_dstip_table"]
	if each:
		file_name = csv_dir+"/"+CSV_TOP_BY_DSTIP_CHART
		_write_csv(file_name, each["header"], each["fields"])

	each = tableinfo["topnum_by_dstip_geo_table"]
	if each:
		file_name = csv_dir+"/"+CSV_TOP_BY_DSTIP_GEO_CHART
		_write_csv(file_name, each["header"], each["fields"])
