...
#_report_files = glob.glob(_TESTDIR_+"/cs2_*.json")

for each in _files:
	os.remove(each)

from priv_module_helpers.report_helpers.mako.analyst_report import crowdstrike2 as target

class TestReportMaker(unittest.TestCase):
	"""test class of ReportMaker in crowdstrike2.py
	"""

	def setUp(self):
		testfile = _TESTDIR_+"/cs2_{}.json"
...
class TestEditorMaker(unittest.TestCase):
	"""test class of EditorMaker in crowdstrike2.py
	"""
