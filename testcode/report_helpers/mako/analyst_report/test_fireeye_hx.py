...
_report_search_files = glob.glob(_TESTDIR_+"/hxalert*.csv")

for each in _files:
	os.remove(each)

from priv_module_helpers.report_helpers.mako.analyst_report import fireeye_hx as target

class TestReportMaker(unittest.TestCase):
	"""test class of ReportMaker in fireeye_hx.py
	"""

	def setUp(self):
		#testfile = _TESTDIR_+"/hxalert{}.json"
		testfile = _TESTDIR_+"/hxalert_v2_{}.json"
		with open(testfile.format("1")) as f:
			self.reportdata = json.load(f)
		#with open(testfile.format("1")) as f:
		#	self.reportdata1 = json.load(f)
		#with open(testfile.format("2")) as f:
		#	self.reportdata2 = json.load(f)
		#with open(testfile.format("3")) as f:
		#	self.reportdata3 = json.load(f)
		#with open(testfile.format("4")) as f:
		#	self.reportdata4 = json.load(f)
		#with open(testfile.format("5")) as f:
	...
	def test_write(self):
		...
		self.assertEqual(code, 0)
		if not self.reportdata.has_key("with_eng"):
			self.reportdata["with_eng"] = "1"
...
class TestEditorMaker(unittest.TestCase):
	"""test class of EditorMaker in fireeye_hx.py
	"""
	...
	def test_render(self):
		...
		#self.write_file(doc, _OUTPUT_DIR+"/editor2.html")
		#self.assertIsInstance(doc, basestring)
		#doc = target.EditorMaker.to_html(self.reportdata3)
		#self.write_file(doc, _OUTPUT_DIR+"/editor3.html")
		#self.assertIsInstance(doc, basestring)
		#doc = target.EditorMaker.to_html(self.reportdata4)
		#self.write_file(doc, _OUTPUT_DIR+"/editor4.html")
		#self.assertIsInstance(doc, basestring)
		#doc = target.EditorMaker.to_html(self.reportdata5)
		#self.write_file(doc, _OUTPUT_DIR+"/editor5.html")
		#self.assertIsInstance(doc, basestring)
